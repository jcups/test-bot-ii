...
public class PhotoCommand extends BotCommand {
    ...
    public PhotoCommand(UnsplashService service) {
        super("/photo", "Получить случайное фото");
    ...
    public void execute(AbsSender absSender, User user, Chat chat, String[] arguments) {
        if (arguments.length == 0) {
            service.getRandom(String.valueOf(chat.getId()), absSender);
        } else {
            service.searchByTag(String.valueOf(chat.getId()), arguments[0], absSender);
        }
    }
}
